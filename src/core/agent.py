...
from tools.vulnerability_scanner import EnhancedVulnerabilityScanner
...
'''
    ...
    def run_vulnerability_scanning(self, targets):
        """Run enhanced vulnerability scanning with false positive filtering"""
        print("🔄 Running enhanced vulnerability scanning with false positive filtering...")
        
        # Initialize enhanced scanner
        enhanced_scanner = EnhancedVulnerabilityScanner(max_workers=self.config.workers)
        
        # Run scans with verification
        scan_results = enhanced_scanner.scan_targets_with_verification(targets)
        
        # Print comprehensive summary
        summary = scan_results['summary']
        print(f"\n📊 ENHANCED SCAN SUMMARY:")
        print(f"   • Raw findings: {summary['total_raw_findings']}")
        print(f"   • Verified vulnerabilities: {summary['verified_count']}")
        ...
        print(f"   • High-confidence vulnerabilities: {summary['high_confidence_vulns']}")
        
        return scan_results['verified_vulnerabilities']
    ...
    async def run_ai_guided_vulnerability_scans(self, analysis_results: Dict):
        ...
        # NEW: Run comprehensive LFI tests
        print("   📁 Running comprehensive LFI tests...")
        lfi_vulns = await self.vuln_scanner.run_comprehensive_lfi_tests(
            analysis_results.get('all_targets', [''])[0], crawled_urls
        )
        
        # DEBUG: Check what LFI findings we got
        print(f"   🔧 DEBUG: LFI tests found {len(lfi_vulns)} vulnerabilities")
        if lfi_vulns:
            for i, lfi_vuln in enumerate(lfi_vulns[:3]):  # Show first 3
                print(f"   🔧 DEBUG LFI {i}: {lfi_vuln.get('type')} - {lfi_vuln.get('url')}")
        
        # Combine ALL findings (make sure lfi_vulns is included)
        all_findings = (
            priority_findings + other_findings + param_vulns + 
            auth_vulns + injection_vulns + logic_vulns + lfi_vulns  # ✅ MAKE SURE THIS IS INCLUDED
        )
        
        # DEBUG: Show total findings count
        print(f"   🔧 DEBUG: Total raw findings: {len(all_findings)}")
...
"""
    ...
    async def _run_enhanced_dir_scan(self, url: str) -> List[Dict]:
        ...
        if not available_wordlists:
            self.logger.warning("No directory brute-force wordlists found")
            return findings
        ...
        for wordlist in available_wordlists[:2]:  # Use first 2 available wordlists
            try:
                        ...
                        if result.get('status') in [200, 301, 302, 403]:
                            ...
                            for lfi_file in lfi_files:
                                if lfi_file in filename:
                                    ...
                                    findings.append(finding)
                                    self.logger.info(f"✅ Sensitive file found: {filename}")
                ...
                self.logger.debug(f"Directory scan with {wordlist} failed: {e}")
...
# NEW: Batch processing methods - ADD EVERYTHING BELOW THIS LINE
    ...
    async def run_ai_guided_vulnerability_scans(self, analysis_results: Dict):
        """Run comprehensive vulnerability scans with ALL new test types"""
        priority_targets = analysis_results.get('priority_targets', [])
        all_targets = analysis_results.get('all_targets', [])
        ai_recommendations = analysis_results.get('recommended_tests', [])
        ...
        # Get crawled URLs for advanced testing
        crawled_urls = []
        recon_file = self.scan_data_dir / "crawled_urls.txt"
        if recon_file.exists():
            with open(recon_file, 'r') as f:
        ...
        # NEW: Run comprehensive authentication tests
        print("   🔐 Running authentication security tests...")
        auth_vulns = await self.vuln_scanner.run_authentication_tests(analysis_results.get('all_targets', [''])[0])
        
        # NEW: Run advanced injection tests
        print("   🎯 Running advanced injection tests...")
        injection_vulns = await self.vuln_scanner.run_advanced_injection_tests(
            analysis_results.get('all_targets', [''])[0], crawled_urls
        )
        
        # NEW: Run business logic tests
        print("   💼 Running business logic tests...")
        logic_vulns = await self.vuln_scanner.run_business_logic_tests(
            analysis_results.get('all_targets', [''])[0], crawled_urls
        )
        
        # Run parameter-based tests
        print("   🎯 Running parameter-based vulnerability tests...")
        param_vulns = await self.vuln_scanner.run_parameter_tests_async(
            analysis_results.get('all_targets', [''])[0], crawled_urls
        )
        
        # Scan priority targets with enhanced comprehensive tests
        print(f"   🔍 Scanning {len(priority_targets)} priority targets with COMPREHENSIVE tests...")
        priority_findings = await self._scan_targets_parallel(
            priority_targets, enhanced=True, batch_name="priority", crawled_urls=crawled_urls
        )
        ...
        if other_targets:
            print(f"   🌐 Scanning {len(other_targets)} additional targets with basic tests...")
            other_target_objs = [{'url': url, 'priority': 'medium', 'score': 1} for url in other_targets]
            other_findings = await self._scan_targets_parallel(
                other_target_objs, enhanced=False, batch_name="additional", crawled_urls=crawled_urls
            )
        else:
            other_findings = []
        ...
        # Combine ALL findings (add lfi_vulns to the list)
        all_findings = (
            priority_findings + other_findings + param_vulns + 
            auth_vulns + injection_vulns + logic_vulns + lfi_vulns  # ADD THIS
        )
        ...
        all_findings = (
            priority_findings + other_findings + param_vulns + 
            auth_vulns + injection_vulns + logic_vulns
        )
    ...
    async def _scan_single_target(self, target: Dict, enhanced: bool = False, crawled_urls: List[str] = None) -> List[Dict]:
        """Enhanced single target scanning with all new test types"""
        findings = []
        url = target['url']
        
        try:
            if enhanced:
                # Use comprehensive testing for high-priority targets
                findings = await self.vuln_scanner.run_comprehensive_tests(target, crawled_urls or [])
            else:
                # Use basic testing for other targets
    ...
    async def _scan_targets_parallel(self, targets: List[Dict], enhanced: bool = False, batch_name: str = "batch", crawled_urls: List[str] = None) -> List[Dict]:
        """Enhanced parallel scanning with crawled URLs support"""
        all_findings = []
        total_targets = len(targets)
        ...
        async def bounded_scan(target):
            async with semaphore:
                return await self._scan_single_target(target, enhanced, crawled_urls)
