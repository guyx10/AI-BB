...
class AIStrategist:
    ...
    async def _analyze_with_model(self, scan_data: Dict, model: str, analysis_type: str):
...
JSON format:"""
    ...
    def _build_executive_summary_prompt(self, report_data: Dict) -> str:
        ...
        # Group vulnerabilities by type for better analysis
        vuln_by_type = {}
        ...
        prompt = f"""Create a CONCISE executive summary for technical management:
...
{json.dumps(vuln_by_type, indent=2)}

Focus on:
1. BUSINESS RISK - What can actually be exploited?
2. URGENT ACTIONS - What needs fixing this week?
3. EXPLOITATION SCENARIOS - How could attackers use these?
4. COMPLIANCE IMPACT - Any regulatory concerns?

Avoid generic statements like "implement security best practices". Be specific about:

- Which vulnerabilities allow remote code execution?
- Which ones expose customer data?
- What's the realistic attack timeline?

Format as SPECIFIC, ACTIONABLE JSON:"""
    ...
    def _parse_executive_summary(self, response_text: str, report_data: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            if not ai_summary:
                print("❌ No JSON found in AI executive summary response")
                return self._generate_basic_executive_summary(report_data)
